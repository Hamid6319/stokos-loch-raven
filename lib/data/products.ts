export const PRODUCTS = [
  {
    id: 1,
    category: "pizzas",
    title: "Tomato & Cheese Pizza",
    price: "10.99",
    oldPrice: null,
    image: "/images/pizza1.jpeg",
    badge: "Best Seller",
    description: "Classic cheese or create your own pizza.",
    sizes: [
      { label: "Medium 12\"", price: 10.99 },
      { label: "Large 14\"", price: 11.99 },
      { label: "X-Large 16\"", price: 12.99 },
    ]
  },
  {
    id: 2,
    category: "pizzas",
    title: "Honey BBQ Chicken Pizza",
    price: "12.99",
    image: "/images/pizza1.jpeg",
    badge: null,
    description: "Topped with BBQ sauce, mozzarella, and chicken meat.",
    sizes: [
      { label: "Medium 12\"", price: 12.99 },
      { label: "Large 14\"", price: 14.99 },
      { label: "X-Large 16\"", price: 17.99 },
    ]
  },
  {
    id: 3,
    category: "pizzas",
    title: "Buffalo Chicken Pizza",
    price: "12.99",
    image: "/images/pizza1.jpeg",
    badge: null,
    description: "Crispy chicken strips, our own wing sauce, bleu cheese, and mozzarella cheese.",
    sizes: [
              { label: "Medium 12\"", price: 12.99 },
      { label: "Large 14\"", price: 14.99 },
      { label: "X-Large 16\"", price: 17.99 },
    ]
  },
  {
    id: 4,
    category: "pizzas",
    title: "Stoko's Deluxe Pizza",
    price: "16.98",
    oldPrice: null,
    image: "/images/pizza1.jpeg",
    badge: null,
    description: "Pepperoni, sausage, ground beef, ham, bacon, mushrooms, green peppers, onions, and black olives.",
    sizes: [
      { label: "Medium 14\"", price: 16.98 },
      { label: "Large 14\"", price: 18.98 },
      { label: "X-Large 16\"", price: 20.99 },
    ]
  },

    {
    id: 5,
    category: "pizzas",
    title: "Greek Deluxe Pizza",
    price: "15.99",
    oldPrice: null,
    image: "/images/pizza1.jpeg",
    badge: null,
    description: "Feta cheese, gyro meat, extra cheese, tomatoes, green peppers, onions, black olives.",
    sizes: [
      { label: "Medium 14\"", price: 15.99 },
      { label: "Large 14\"", price: 17.98 },
      { label: "X-Large 16\"", price: 18.98 },
    ]
  },

      {
    id: 6,
    category: "pizzas",
    title: "Veggie Deluxe Pizza",
    price: "16.98",
    oldPrice: null,
    image: "/images/pizza1.jpeg",
    badge: null,
    description: "Mushrooms, onions, green peppers, black olives, tomatoes, and extra cheese.",
    sizes: [
      { label: "Medium 14\"", price: 16.98 },
      { label: "Large 14\"", price: 17.98 },
      { label: "X-Large 16\"", price: 18.98 },
    ]
  },

       {
    id: 7,
    category: "pizzas",
    title: "Meat Deluxe Pizza",
    price: "17.98",
    oldPrice: null,
    image: "/images/pizza1.jpeg",
    badge: null,
    description: "",
    sizes: [
      { label: "Medium 14\"", price: 17.98 },
      { label: "Large 14\"", price: 18.98 },
      { label: "X-Large 16\"", price: 19.99 },
    ]
  },

       {
    id: 8,
    category: "pizzas",
    title: "Cheesesteak Pizza",
    price: "12.99",
    oldPrice: null,
    image: "/images/pizza1.jpeg",
    badge: null,
    description: "Mushrooms, onions, green peppers, black olives, tomatoes, and extra cheese.",
    sizes: [
      { label: "Medium 14\"", price: 12.99 },
      { label: "Large 14\"", price: 14.99 },
      { label: "X-Large 16\"", price: 17.98 },
    ]
  },

  

       {
    id: 9,
    category: "pizzas",
    title: "Chicken Alfredo Pizza",
    price: "17.98",
    oldPrice: null,
    image: "/images/pizza1.jpeg",
    badge: null,
    description: "Grilled chicken breast, fresh garlic and Alfredo sauce topped with mozzarella cheese.",
    sizes: [
      { label: "Medium 14\"", price: 17.98 },
      { label: "Large 14\"", price: 18.98 },
      { label: "X-Large 16\"", price: 19.98 },
    ]
  },

       {
    id: 10,
    category: "pizzas",
    title: "Hawaiian Pizza",
    price: "13.99",
    oldPrice: null,
    image: "/images/pizza1.jpeg",
    badge: null,
    description: "Pineapple, imported ham.",
    sizes: [
      { label: "Medium 14\"", price: 13.99 },
      { label: "Large 14\"", price: 15.99 },
      { label: "X-Large 16\"", price: 16.99 },
    ]
  },

];